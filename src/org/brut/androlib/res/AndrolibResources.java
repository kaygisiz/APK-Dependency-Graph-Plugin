 ...
 */
package org.brut.androlib.res;

import com.intellij.openapi.progress.ProgressIndicator;
import com.intellij.openapi.progress.ProgressManager;
import org.brut.androlib.AndrolibException;
import org.brut.androlib.ApkOptions;
import org.brut.androlib.err.CantFindFrameworkResException;
import org.brut.androlib.meta.MetaInfo;
import org.brut.androlib.meta.PackageInfo;
import org.brut.androlib.meta.VersionInfo;
import org.brut.androlib.res.data.*;
import org.brut.androlib.res.decoder.*;
import org.brut.androlib.res.decoder.ARSCDecoder.ARSCData;
import org.brut.androlib.res.decoder.ARSCDecoder.FlagsOffset;
import org.brut.directory.ExtFile;
import org.brut.androlib.res.util.ExtMXSerializer;
import org.brut.androlib.res.util.ExtXmlSerializer;
import org.brut.androlib.res.xml.ResValuesXmlSerializable;
import org.brut.androlib.res.xml.ResXmlPatcher;
import org.brut.common.BrutException;
import org.brut.directory.Directory;
import org.brut.directory.DirectoryException;
import org.brut.directory.FileDirectory;
import org.brut.util.Duo;
import org.brut.util.Jar;
import org.brut.util.OS;
import org.brut.util.OSDetection;
...
final public class AndrolibResources {
    private ProgressIndicator progressIndicator = ProgressManager.getInstance().getProgressIndicator();
    ...
    public ResPackage loadMainPkg(ResTable resTable, ExtFile apkFile)
            throws AndrolibException {
        printMessage("Loading resource table...");
    ...
    public ResPackage loadFrameworkPkg(ResTable resTable, int id, String frameTag)
        ...
        printMessage("Loading resource table from file: " + apk);
    ...
    public void decodeManifest(ResTable resTable, ExtFile apkFile, File outDir)
        ...
        try {
            ...
            printMessage("Decoding AndroidManifest.xml with only framework resources...");
    ...
    public void adjustPackageManifest(ResTable resTable, String filePath)
        ...
        if (packageOriginal.equalsIgnoreCase(mPackageRenamed) ||
            ...
            printMessage("Regular manifest package...");
        } else {
            printMessage("Renamed manifest package found! Replacing " + mPackageRenamed + " with " + packageOriginal);
    ...
    public void decodeManifestWithResources(ResTable resTable, ExtFile apkFile, File outDir)
        ...
        try {
            inApk = apkFile.getDirectory();
            out = new FileDirectory(outDir);
            printMessage("Decoding AndroidManifest.xml with resources...");
    ...
    public void decode(ResTable resTable, ExtFile apkFile, File outDir)
        ...
        for (ResPackage pkg : resTable.listMainPackages()) {
            ...
            printMessage("Decoding file-resources...");
            for (ResResource res : pkg.listFiles()) {
                fileDecoder.decode(res, in, out);
            }

            printMessage("Decoding values */* XMLs...");
        ...
        if (! aaptPath.isEmpty()) {
            ...
            if (aaptFile.canRead() && aaptFile.exists()) {
                ...
                if (apkOptions.verbose) {
                    printMessage(aaptFile.getPath() + " being used as aapt location.");
                }
            } else {
    ...
    public File getFrameworkApk(int id, String frameTag)
        ...
        if (id == 1) {
            try (InputStream in = AndrolibResources.class.getResourceAsStream("/org/brut/androlib/android-framework.jar");
                ...
                if (apk.exists() && dir.listFiles().length > 1 && ! apkOptions.forceDeleteFramework) {
                    ...
                    for (File file : dir.listFiles()) {
                        if (file.isFile() && file.getName().endsWith(".apk")) {
                            printMessage("Removing " + file.getName() + " framework file...");
                            file.delete();
                        }
                    }
                }
    ...
    public void installFramework(File frameFile, String tag)
        ...
        try {
            ...
            zip.close();
            printMessage("Framework installed to: " + outFile);
        } catch (IOException ex) {
            throw new AndrolibException(ex);
        } finally {
    ...
    public File getAndroidResourcesFile() throws AndrolibException {
        try {
            return Jar.getResourceAsFile("/org/brut/androlib/android-framework.jar");
        } catch (BrutException ex) {
            throw new AndrolibException(ex);
        }
    }
