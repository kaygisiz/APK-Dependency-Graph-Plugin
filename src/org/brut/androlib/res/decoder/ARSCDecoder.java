...
import org.brut.androlib.res.data.value.*;
import org.brut.util.Duo;
import org.brut.androlib.res.data.ResTable;
import org.brut.util.ExtDataInput;
...
public class ARSCDecoder {
    ...
    private void readLibraryType() throws AndrolibException, IOException {
        ...
        for (int i = 0; i < libraryCount; i++) {
            ...
            String message = String.format("Decoding Shared Library (%s), pkgId: %d", packageName, packageId);
            LOGGER.info(message);
    ...
    private ResTypeSpec readSingleTableTypeSpec() throws AndrolibException, IOException {
        ...
        if (mFlagsOffsets != null) {
            mFlagsOffsets.add(new FlagsOffset(Integer.parseInt(String.valueOf(mCountIn.getCount())), entryCount));
        }
    ...
    private ResType readTableType() throws IOException, AndrolibException {
        ...
        if (position != mCountIn.getCount()) {
            mIn.skipBytes(Integer.parseInt(String.valueOf(position - mCountIn.getCount())));
        }

        if (typeFlags == 1) {
            String message = "Sparse type flags detected: " + mTypeSpec.getName();
            LOGGER.info(message);
            ProgressManager.getInstance().getProgressIndicator().setText(message);
        }
    ...
    public static class Header {
        ...
        public static Header read(ExtDataInput in, CountingInputStream countIn) throws IOException {
            short type;
            int start = Integer.parseInt(String.valueOf(countIn.getCount()));
            try {
                type = in.readShort();
            } catch (EOFException ex) {
                return new Header(TYPE_NONE, 0, 0, Integer.parseInt(String.valueOf(countIn.getCount())));
            }
    ...
    public static class ARSCData {
        ...
        public ResPackage getOnePackage() throws AndrolibException {
            if (mPackages.length <= 0) {
                ...
                String message = "Arsc file contains multiple packages. Using package "
