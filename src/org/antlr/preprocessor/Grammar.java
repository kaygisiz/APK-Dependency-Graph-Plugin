 ...
 *
 * $Id: //depot/code/org.antlr/release/antlr-2.7.0/antlr/preprocessor/Grammar.java#1 $
 */
...
class Grammar {
    ...
    public Grammar(String name, String superGrammar, IndexedVector rules) {
	this.name = name;
	...
	if ((options != null && options.getElement("importVocab") == null) || options == null) {
	    ...
	    String path = org.antlr.Tool.pathToFile(originatingGrFileName);
	    String superExportVocabFileName = path + superG.exportVocab +
		org.antlr.CodeGenerator.TokenTypesFileSuffix +
		org.antlr.CodeGenerator.TokenTypesFileExt;
	    String newImportVocabFileName = org.antlr.Tool.fileMinusPath(superExportVocabFileName);
		...
		try {
		    org.antlr.Tool.copyFile(superExportVocabFileName, newImportVocabFileName);
		} catch (IOException io) {
		    org.antlr.Tool.toolError("cannot find/copy importVocab file " + superExportVocabFileName);
		    return;
		}
	    }
	}
	...
	if ( overriddenRule!=null ) {
		...
		org.antlr.Tool.warning("rule "+getName()+"."+overriddenRule.getName()+
