 ...
 *
 * $Id: //depot/code/org.antlr/release/antlr-2.7.0/antlr/CppCodeGenerator.java#1 $
 */
...
public class CppCodeGenerator extends CodeGenerator {
	...
	private static final String namespaceStd   = "ANTLR_USE_NAMESPACE(std)";
	private static final String namespaceAntlr = "ANTLR_USE_NAMESPACE(antlr)";
	...
	public void gen(ActionElement action) {
		...
		if ( action.isSemPred ) {
			...
			if ( tInfo.assignToRoot ) {
				...
				println("currentAST.child = "+tInfo.refRuleRoot+"!="+namespaceAntlr+"nullAST &&"+tInfo.refRuleRoot+"->getFirstChild()!="+namespaceAntlr+"nullAST ?");
				tabs++;
	...
	public void gen(RuleRefElement rr) {
		...
		{
			println(rr.getLabel() + " = _t==ASTNULL ? "+namespaceAntlr+"nullAST : "+lt1Value+";");
		}
	...
	public void gen(TreeElement t) {
		// save AST cursor
		println(namespaceAntlr+"RefAST __t" + t.ID + " = _t;");
		...
		if (grammar.buildAST) {
			...
			println("currentAST.child = "+namespaceAntlr+"nullAST;");
		}
	...
	public void gen(WildcardElement wc) {
		...
		if (grammar instanceof TreeWalkerGrammar) {
			println("if ( _t=="+namespaceAntlr+"nullAST ) throw "+namespaceAntlr+"MismatchedTokenException();");
		}
				...
				for (int i=0; i<rblk.labeledElements.size(); i++) {
					...
					if (
						...
						if (
							...
							if (grammar.buildAST) {
								println(labeledElementASTType+" " + a.getLabel() + "_AST = "+namespaceAntlr+"nullAST;");
							}
						}
						else {
							if (grammar.buildAST) {
	...
	public void genBody(LexerGrammar g) throws IOException {
		currentOutput = org.antlr.Tool.openOutputFile(grammar.getClassName() + ".cpp");
...
//					println("astFactory.makeASTRoot(currentAST, " + astName + ");");
	...
	public void genInclude(LexerGrammar g) throws IOException {
		currentOutput = org.antlr.Tool.openOutputFile(grammar.getClassName() + ".hpp");
		//SAS: changed for proper text file io
...
//		}
		...
		if (unlabeledUserSpec != null) {
			...
			if ( !(grammar instanceof TreeWalkerGrammar) ) {
				...
				println("if (_t!="+namespaceAntlr+"nullAST) {_t = _t->getNextSibling();}");
			}
			if (grammar.hasSyntacticPredicate) {
				tabs--;
				// When guessing, rethrow exception
...
//		}
	...
	protected void genSynPred(SynPredBlock blk, String lookaheadExpr) {
		...
		// Save input state
		if ( grammar instanceof TreeWalkerGrammar ) {
			println(namespaceAntlr+"RefAST __t" + blk.ID + " = _t;");
		}
		else {
...
//		println("");
	...
	protected void genTokenTypes(TokenManager tm) throws IOException {
		...
		currentOutput = org.antlr.Tool.openOutputFile(tm.getName() + TokenTypesFileSuffix+".hpp");
		//SAS: changed for proper text file io
